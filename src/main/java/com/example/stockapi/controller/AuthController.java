...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody Map<String, String> payload) {
        ...
        for (String s : roleIndividual) {
            ...
            switch (s) {
                case "USER" -> {
                    ...
                    try {
                        userTempRole = roleRepository.findByName(ERole.ROLE_USER)
                ...
                case "ADMIN" -> {
                    Role adminTempRole;
                    try {
                        adminTempRole = roleRepository.findByName(ERole.ROLE_ADMIN)
                                .orElseThrow(() -> new RuntimeException("No such role"));
                    } catch (RuntimeException re) {
        ...
        User user = new User(payload.get("name"), payload.get("username"),
                payload.get("mobilenumber"), encoder.encode(payload.get("password")));
    ...
    public ResponseEntity<?> deleteUser(HttpServletRequest httpServletRequest,
        ...
        User u = userRepository
                .getUserByEmail(jwtUtils.getUserNameFromJwtToken(jwtUtils.getJwtFromCookies(httpServletRequest)))
                .orElseThrow();
        ...
        try {
            this.authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(u.getEmail(), p));
